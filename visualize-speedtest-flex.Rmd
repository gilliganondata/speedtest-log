...
get_plot_ts <- function(test_data, timespan = c(1,100) , granularity = "Hour",
  ...
  test_data <- test_data %>% 
    mutate(test_time = force_tz(test_time, timezone))
  ...
  # test_data <- test_data %>%
  #   mutate(test_time = ifelse(granularity == "Hour", test_time, 
  #                             ifelse(granularity == "Day", as.Date(test_time), test_time))) %>%
  #   mutate(test_time = as.POSIXct(test_time))
  ...
  gg <- ggplot(data_viz_df, 
    ...
    labs(title = paste("The Median", label_title, "Speed Was", 
...
gg_upload <- get_plot_ts(upload_data)
...
get_download_ts <- reactive({
  ...
  get_plot_ts(download_data, timespan, granularity, min_max, quartiles_2_3)
})  
...
# output$min_max_value <- renderPrint({input$min_max})
# 
# verbatimTextOutput("min_max_value")
...
get_upload_ts <- reactive({
  ...
  min_max <- input$min_max
  quartiles_2_3 <- input$quartiles_2_3
  get_plot_ts(upload_data, timespan, granularity, min_max, quartiles_2_3)
})  
